-- Migration: Fix recommended matches orientaton/gender matching to be reciprocal
...
BEGIN
    ...
    WHERE p.id <> caller_id
      -- Strictly filter to auth.users (reals only), verified liveness only, and active market status
      AND p.id IN (SELECT au.id FROM auth.users au)
      AND p.liveness_verified = TRUE
      AND p.status = 'ACTIVE'
      -- 1. Target matches caller's preference
      AND (
      ...
      -- 2. Caller matches target's preference (Reciprocity)
      AND (
        (p.orientation = 'bi') OR
        (p.orientation = 'hetero' AND caller_gender <> p.gender) OR
        (p.orientation = 'homo' AND caller_gender = p.gender)
      )
