...
serve(async (req) => {
  ...
  try {
    ...
    const avgCandidateE =
      candidates.reduce((sum: number, c: Candidate) => sum + c.extraversion, 0) / candidates.length;
    ...
    let poolMatches = compatiblePool.filter(
      (c: Candidate) => isGlobal || c.distanceKm === undefined || c.distanceKm <= currentRadius,
    );
    ...
    if (!isGlobal && poolMatches.length < targetThreshold) {
      ...
      while (poolMatches.length < targetThreshold && currentRadius < 500) {
        ...
        poolMatches = compatiblePool.filter(
          (c: Candidate) => c.distanceKm === undefined || c.distanceKm <= currentRadius,
        );
      }
    ...
    const scoredMatches = poolMatches.map((candidate: Candidate) => {
    ...
    scoredMatches.sort(
      (a: { ev_score: number }, b: { ev_score: number }) => b.ev_score - a.ev_score,
    );
