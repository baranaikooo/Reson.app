...
export const SCENARIO_VARIANTS: Record<DimensionId, Scenario[]> = {
  ...
  q2: [
    ...
    { id: "q2", title: "Rozhodnutie sestry",
  ...
  q4: [
    { id: "q4", title: "Sám alebo s druhými",
  ...
  q5: [
    { id: "q5", title: "Pravda alebo dobrá vec",
    ...
    { id: "q5", title: "Priateľ v probléme",
  ...
  q6: [
    { id: "q6", title: "Talent alebo drina",
...
export const SCENARIOS: ReadonlyArray<Scenario> = (Object.keys(SCENARIO_VARIANTS) as DimensionId[]).map(
  (k) => SCENARIO_VARIANTS[k][0]
);
...
export function calcResonance(a: FullAnswers, b: FullAnswers, customWeights?: DynamicWeights): number {
...
export function updateWeightsFromFeedback(feedback: UserFeedback): void {
  ...
  const dimensionFeedback: Partial<Record<DimensionId, { weightedSum: number; totalWeight: number }>> = {};
  const now = Date.now();
...
export function calcDimensionScores(a: FullAnswers, b: FullAnswers): Partial<Record<DimensionId, number>> {
...
export function selectMatchPattern(userAnswers: FullAnswers, availablePatterns: MatchPattern[]): MatchPattern {
  const now = Date.now();
  ...
  availablePatterns.forEach(pattern => {
    ...
    const exploration = arm.pulls > 0
      ...
      : 1.0; // High exploration for untried arms
...
export function getMatchPattern(a: FullAnswers, b: FullAnswers): MatchPattern {
  return Object.values(a).map((ans, i) => {
...
export function calcResonanceWithUncertainty(a: FullAnswers, b: FullAnswers): {
  score: number;
  uncertainty: number;
  ...
  const feedbackCount = userFeedbackHistory.filter(f =>
    f.dimensionScores && Object.keys(f.dimensionScores).some(dim =>
      dimensionScores[dim] === f.dimensionScores![dim]
    )
  ).length;
...
export function resonanceBreakdown(a: FullAnswers, b: FullAnswers): BreakdownItem[] {
  const labels: Record<string, { sim: string; comp: string }> = {
    q1: { sim: "Rozhodujete sa rovnakým tempom",      comp: "Jeden z vás brzdí, druhý tlačí — vyvažujete sa" },
    q2: { sim: "Obaja idete rovnako — hlavou alebo srdcom", comp: "Vidíte ľudí cez iné šošovky" },
    q3: { sim: "K riziku pristupujete rovnako",       comp: "Jeden drží istotu, druhý skúša nové — dobrá kombinácia" },
    q4: { sim: "Rovnako vnímate samotu aj blízkosť",   comp: "Inak chápete, kedy sa oprieť o druhých" },
    q5: { sim: "Máte rovnaký kompas v zložitých veciach", comp: "Rozdielne vnímate pravdu a dobro" },
  ...
  (Object.keys(DIMENSIONS) as Array<DimensionId>).forEach((k) => {
    ...
    else if (mode === "complement" && !same) out.push({ kind: "complement", label: labels[k].comp });
  });
  return out;
}
...
const PROMPT_BANK: Record<"q1same"|"q1diff"|"q2same"|"q2diff"|"q3same"|"q3diff"|"q4same"|"q4diff"|"q5same"|"q5diff"|"q6same"|"q6diff", Prompt[]> = {
  q1same: [
    { title: "Tempo života", sub: "Obaja sa rozhodujete podobne. Kedy ste naposledy svoj štýl ľutovali a kedy vám priniesol víťazstvo?" },
    { title: "Rýchle áno", sub: "Ak by ste sa mali dnes večer rozhodnúť pre niečo veľké, čo by to bolo a prečo by ste neváhali?" },
    { title: "Cena rozhodnutia", sub: "Aké rozhodnutie vás stálo najviac — a urobili by ste ho dnes znova?" },
  ],
  q1diff: [
    { title: "Brzda a plyn", sub: "Jeden z vás skáče, druhý si veci premyslí. Spomeňte si na moment, keď vám ten druhý prístup zachránil situáciu." },
    { title: "Spoločný plán", sub: "Predstavte si, že spolu plánujete víkend o hodinu — ako sa dohodnete, kto rozhoduje?" },
    { title: "Risk vs. premýšľanie", sub: "Kedy bola vaša impulzívnosť (alebo opatrnosť) presne to, čo treba?" },
  ],
  q2same: [
    { title: "Hlava alebo srdce", sub: "Spomeňte si na situáciu, keď ste museli vybrať medzi tým, čo dáva zmysel, a tým, čo cítite. Ako ste sa rozhodli?" },
    { title: "Empatia v praxi", sub: "Kedy vás emócia (alebo logika) doviedla k najlepšiemu rozhodnutiu vášho života?" },
    { title: "Človek v kríze", sub: "Ako si predstavujete, že vás partner podrží, keď budete na dne — slovami alebo riešením?" },
  ],
  q2diff: [
    { title: "Iné šošovky", sub: "Vidíte ľudí cez iné šošovky. Skúste si vzájomne opísať jednu spoločnú osobu — čo si na nej všímate?" },
    { title: "Cit vs. analýza", sub: "Kedy vám rozdielny pohľad pomohol pochopiť situáciu, ktorú sami zvládnuť neviete?" },
    { title: "Most medzi nami", sub: "Ako môže človek so srdcom pomôcť človeku s hlavou — a naopak?" },
  ],
  q3same: [
    { title: "Istota alebo risk", sub: "K riziku pristupujete rovnako. Aké je najodvážnejšie (alebo najopatrnejšie) rozhodnutie, na ktoré ste hrdí?" },
    { title: "Komfortná zóna", sub: "Kde leží vaša komfortná zóna — a kedy ste z nej naposledy vykročili?" },
    { title: "Spoločná cesta", sub: "Keby ste mali spolu vyraziť do neznáma, kam by to bolo a kto by viedol?" },
  ],
  q3diff: [
    { title: "Istota a zmena", sub: "Jeden má rád stabilitu, druhý zmenu. Ako by ste si rozdelili roly v spoločnom projekte?" },
    { title: "Volant", sub: "Keby ste teraz mali spolu vyraziť na nečakaný výlet, kto sedí za volantom a prečo?" },
    { title: "Nový krok", sub: "Aký bol váš posledný skok do neznáma — alebo váš posledný rozumný odklad?" },
  ],
  q4same: [
    { title: "Sám alebo spolu", sub: "Rovnako vnímate samotu aj blízkosť. Kedy je pre vás samota liekom a kedy je to spolu?" },
    { title: "Opora", sub: "Kto bol naposledy vaša opora — a kedy ste vy boli oporou pre niekoho?" },
    { title: "Hranice", sub: "Kde sú vaše hranice — kedy ešte \u201Esom tu pre teba\u201C a kedy už \u201Epotrebujem byť sám/sama\u201C?" },
  ],
  q4diff: [
    { title: "Požiadať o pomoc", sub: "Vnútornú silu vnímate rozdielne. Povedzte si moment, keď vám prosba o pomoc niečo zmenila — alebo keď ste ju neuniesli." },
    { title: "Spoločná batoha", sub: "Aké veci si nesiete sami — a ktoré by ste boli ochotní niekomu odovzdať?" },
    { title: "Nezávislosť a blízkosť", sub: "Ako vyzerá vaša ideálna rovnováha medzi \u201Emy\u201C a \u201Eja\u201C?" },
  ],
  q5same: [
    { title: "Pravda alebo dobro", sub: "Zhodli ste sa na podobnom kompase. Kde je u vás hranica, ktorú by ste neprekročili ani pre väčšie dobro?" },
    { title: "Malé klamstvá", sub: "Existuje pre vás \u201Edobré klamstvo\u201C? Skúste si vzájomne dať príklad." },
    { title: "Vlastná česť", sub: "Aký princíp si dnes držíte aj vtedy, keď je to pre vás nevýhodné?" },
  ],
  q5diff: [
    { title: "Pravda vs. výsledok", sub: "Rozdielne vnímate pravdu a dobro. Spomeňte si na situáciu, kde by ste sa rozhodli inak — a vysvetlite prečo." },
    { title: "Šedá zóna", sub: "Kde leží vaša šedá zóna — a kde leží tá druhého?" },
    { title: "Skúška charakteru", sub: "Čo by vám muselo byť na váhe, aby ste porušili vlastný princíp?" },
  ],
  q6same: [
    { title: "Talent a drina", sub: "Rovnako vnímate snahu aj talent. V čom ste sa vy sami naučili niečo \u201Eproti svojej prirodzenosti\u201C?" },
    { title: "Zlyhania", sub: "Aké zlyhanie vás najviac posunulo a čo by ste si dnes povedali vtedajšiemu sebe?" },
    { title: "Rast", sub: "Kde dnes cítite, že rastiete — a kde sa cítite zaseknutí?" },
  ],
  q6diff: [
    { title: "Predispozícia vs. úsilie", sub: "Inak sa pozeráte na to, čo nás formuje. Ako by ste viedli dieťa, ktoré niečo veľmi chce, ale nemá k tomu \u201Edary\u201C?" },
    { title: "Cudzia disciplína", sub: "Obdivujete na druhom skôr jeho prirodzený dar alebo to, ako tvrdo pracoval?" },
    { title: "Vlastný príbeh", sub: "Vďačíte za to, kde dnes ste, viac svojej drine alebo svojim danostiam?" },
  ],
};
...
export function catalystFor(a: FullAnswers, b: FullAnswers, seed: string = ""): Prompt {
  ...
  if (a.q2 === b.q2) push("q2same", 3); else push("q2diff", 3);
  if (a.q5 === b.q5) push("q5same", 2.5); else push("q5diff", 2.5);
  if (a.q1 === b.q1) push("q1same", 1.5); else push("q1diff", 2);
  if (a.q3 === b.q3) push("q3same", 1.5); else push("q3diff", 2);
  if (a.q4 === b.q4) push("q4same", 1.5); else push("q4diff", 2);
  if (a.q6 === b.q6) push("q6same", 1); else push("q6diff", 1);
  ...
  for (const c of candidates) { if (r < c.weight) { chosen = c.key; break; } r -= c.weight; }
...
export function isCompatible(me: UserProfile, other: UserProfile): boolean {
  const a = ageBounds(me.age), b = ageBounds(other.age);
...
export type MockMatch = {
  ...
  bio: string; img: string; answers: FullAnswers;
  coords?: { lat: number; lon: number };
  // New EV variables
...
export function calcEVScore(
  depthP: number, depthQ: number,
  consP: number, consQ: number,
  extP: number, extQ: number,
  styleP: string, styleQ: string,
  rtP: number, rtQ: number,
  targetT: number = 1.0,
...
export function orbSeedFor(a: FullAnswers): OrbSeed {
  ...
  return { hue: arch.hue, saturation, speed, turbulence, hash: (h % 1000) / 1000, archetype: arch.id };
}
